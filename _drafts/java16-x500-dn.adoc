...
Dans le projet en question, l'utilisateur peut téléverser un certificat qu'on lit et analyse coté serveur et dont on stocke des métadonnées, 
...
Pour ne pas avoir ce type de problème, il faudrait pas récupérer directement le text ou le principal à partir du certificat, mais passer par un X500Principal.
...
Les résultats montre la sensibilité au format.
...
Le code ci-dessous sert à créer un nouvelle utilisateur en téléversant un certificat.
